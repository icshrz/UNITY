...
class MailBox():
    ...
    def prep_input_mails(self, mfg, use_pinned_buffers=False):
        for i, b in enumerate(mfg):
            if use_pinned_buffers:
                ...
                time_cpu_gather_start = time.time()

                A = self.node_memory[b.srcdata['ID'].long()]
                B = self.node_memory_ts[b.srcdata['ID'].long()]
                C = self.mailbox[b.srcdata['ID'].long()].reshape(b.srcdata['ID'].shape[0], -1)
                D = self.mailbox_ts[b.srcdata['ID'].long()]
                time_cpu_gather_end = time.time()

                b.srcdata['mem'] = A
                b.srcdata['mem_ts'] = B
                b.srcdata['mem_input'] = C
                b.srcdata['mail_ts'] = D
                ...
                # # E = self.node_memory.to('cpu')
                # # F = self.node_memory_ts.to('cpu')
                # # G = self.mailbox.to('cpu')
                # # H = self.mailbox_ts.to('cpu')
                # # time_cpu_gather_start_cpu = time.time()
                # # E = E[b.srcdata['ID'].long()]
                # # F = F[b.srcdata['ID'].long()]
                # # G = G[b.srcdata['ID'].long()].reshape(b.srcdata['ID'].shape[0], -1)
                # # H = H[b.srcdata['ID'].long()]
                # # time_cpu_gather_end_cpu = time.time()

                # time_D2H_start = time.time()
                # A = A.to('cpu')
                # B = B.to('cpu')
                # C = C.to('cpu')
                # D = D.to('cpu')
                # time_D2H_end = time.time()

                # time_H2D_start = time.time()
                # A = A.to('cuda')
                # B = B.to('cuda')
                # C = C.to('cuda')
                # D = D.to('cuda')
                # time_H2D_end = time.time()

                print(f"Time taken for cpu_gather_through_gpu: {time_cpu_gather_end - time_cpu_gather_start:.4f} seconds")
                # print(f"Time taken for cpu_gather: {time_cpu_gather_end_cpu - time_cpu_gather_start_cpu:.4f} seconds")

                # print(f"Time taken for H2D: {time_H2D_end - time_H2D_start:.4f} seconds")
                # print(f"Time taken for D2H: {time_D2H_end - time_D2H_start:.4f} seconds")
    ...
    def update_mailbox(self, nid, memory, root_nodes, ts, edge_feats, block, neg_samples=1):
        with torch.no_grad():
            ...
            print(f"Memory device: {memory.device}")
            # TGN/JODIE
...
#                    mail_ts = scatter(mail_ts, idx, reduce='mean')
                    # 获取 idx 中每个索引的计数
                    idx_count = torch.bincount(idx, minlength=mail.size(0))

                    # 将 idx 对应位置的 mail 值加起来
                    mail_sum = torch.zeros_like(mail).scatter_add_(0, idx.unsqueeze(-1).expand_as(mail), mail)

                    # 计算均值，避免除以零
                    mail = mail_sum / idx_count.clamp_min(1).unsqueeze(-1)
                    
                    # 获取 idx 中每个索引的计数
                    idx_count_ts = torch.bincount(idx, minlength=mail_ts.size(0))

                    # 将 idx 对应位置的 mail_ts 值加起来
                    mail_ts_sum = torch.zeros_like(mail_ts).scatter_add_(0, idx.unsqueeze(-1).expand_as(mail_ts), mail_ts)

                    # 计算均值，避免除以零
                    mail_ts = mail_ts_sum / idx_count_ts.clamp_min(1).unsqueeze(-1)
...
class GRUMemeoryUpdater(torch.nn.Module):
    ...
    def forward(self, mfg):
        ...
        for b in mfg:
            ...
            time_update_start = time.time()
            # print(f"unique mems1: {torch.unique(b.srcdata['mem'], dim=0).shape[0]}")
            ...
            # print(f"nm: {updated_memory.shape[0]}, id shape: {b.srcdata['h'].shape[0]}")
            # print(f"unique fts: {torch.unique(b.srcdata['mem_input'], dim=0).shape[0]}")
            # print(f"unique mems2: {torch.unique(updated_memory, dim=0).shape[0]}")
