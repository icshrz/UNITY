...
for e in range(train_param['epoch']):
    ...
    # 打乱数据
    shuffled_df = df[:train_edge_end].sample(frac=1).reset_index(drop=True)
    ...
    with tqdm(total=num_batches, desc=f'Epoch {e+1}/{train_param["epoch"]}', unit='batch') as pbar:
        for rows in get_batches(shuffled_df, train_param['batch_size']):
            t_tot_s = time.time()
            ...
            pbar.update(1)
            # 显示当前损失
            current_loss = total_loss / (pbar.n * train_param['batch_size'])
