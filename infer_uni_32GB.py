...
def flatten(nested_list):
    ...
    for item in nested_list:
        if isinstance(item, list):  # 如果元素是子列表，递归调用
...
def csr(list1, list2):
    # 将list2展平并转换为集合，便于快速查找
    set2 = set(flatten(list2))
    
    # 展平list1
    flat_list1 = flatten(list1)
    
    # 统计list1中与list2相同的元素数量
    common_elements = [item for item in flat_list1 if item in set2]
    
    # 计算相同元素的占比
    similarity_ratio = len(common_elements) / len(flat_list1) if len(flat_list1) > 0 else 0
...
def sample(rows, idx):
    ...
    tsa = time_sample_end - time_sample_start
    print(f"Time taken for sample {idx}: {tsa:.4f} seconds")
    return nid, eid, mfgs, root_nodes, ts, tsa, ret

def gather(nid, eid, idx, raten, ratee):
    time_gather_start = time.time()
    fm.gather_node_bat(nid, args.data, gnn_dim_node, raten)
    nf, ef = [], []
    for id, n in enumerate(nid):
        nf.append(torch.zeros(*torch.tensor(n).shape, gnn_dim_node).cuda())
    
    if eid != []:
        fm.gather_edge_bat(eid, args.data, gnn_dim_edge, ratee)
        for id, e in enumerate(eid):
            ef.append(torch.zeros(*torch.tensor(e).shape, gnn_dim_edge).cuda())
    else:
        ef = []
...
def infer_task(mfgs, nf, ef, root_nodes, ts, idx, ret):
    
    time_cpy_start = time.time()
    for id, b in enumerate(mfgs):
        b[0].srcdata['h'] = nf[id]
        if ef != []:
            b[0].edata['f'] = ef[id]
    if sample_param['history'] != 1:
        print(f"here")
        for b in mfgs:
            for i in range(len(b)):
                b[i] = b[0]
    time_cpy_end = time.time()
    print(f"Time taken for cpy {idx}: {time_cpy_end - time_cpy_start:.4f} seconds")
    ...
    ap, auc, del_total_loss = compute_aps(pred_pos, pred_neg, root_nodes, ts, idx, ret)
    time_infer_end = time.time()
    ti = time_infer_end - time_infer_start
    print(f"Time taken for infer {idx}: {ti:.4f} seconds")
    
    return ap, auc, del_total_loss, ti

def compute_aps(pred_pos, pred_neg, root_nodes, ts, idx, ret):
    ...
    # print(f"memory: {get_memory_info() / (1024 * 1024):.2f} MB")
    ...
    if mailbox is not None:
        ...
        time_gather_start = time.time() 
        fm.gather_node_bat(eid, args.data, gnn_dim_edge, 0.5)
        mem_edge_feats = torch.zeros(eid[0].shape[0], gnn_dim_edge).cuda()
        time_gather_end = time.time() 
...
for idx, rows in enumerate(get_batches(shuffled_df, train_param['batch_size'])):
    if idx < pre_sample:  # 只执行前 10 个批次的 sample
        ...
        nid, eid, mfgs, root_nodes, ts, tsa, ret = sample(rows, idx)  # 执行 sample 任务
        sample_results[idx] = (nid, eid, mfgs, root_nodes, ts, tsa, ret)  # 保存结果到 sample_results 字典
    else:
        break  # 只执行前 10 个批次
    ...
    with tqdm(total=150, desc=f'Epoch {1}/{train_param["epoch"]}', unit='batch') as pbar:#total=num_batches
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as process_executor:
                with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                    ...
                    nid0, eid0, mfgs0, root_nodes0, ts0, tsa0, ret0 = sample_results[0]
                    nf0, ef0, tg0 = gather(nid0, eid0, 0, 0.0, 0.0)
                    ti, tg, tsa, tc = 0, 0, 0, 0
                    for idx, rows in enumerate(get_batches(shuffled_df, train_param['batch_size'])):
                        if idx >= 150:
                            break
                        nidp, eidp, mfgsp, root_nodesp, tsp, tsap, retp = sample_results[idx]
                        time_cache_start = time.time() 
                        if idx==0:
                            fm.cache_id_features(nid0, eid0, args.data, gnn_dim_node, gnn_dim_edge)
                        else:
                            fm.cache_id_features(nidp, eidp, args.data, gnn_dim_node, gnn_dim_edge)
                        time_cache_end = time.time() 
                        tc = time_cache_end - time_cache_start
                        nidp1, eidp1, mfgsp1, root_nodesp1, tsp1, tsap1, ret1 = sample_results[idx]
                        nidp2, eidp2, mfgsp2, root_nodesp2, tsp2, tsap2, ret2 = sample_results[idx+1]
                        raten = csr(nidp1, nidp2)
                        ratee = csr(eidp1, eidp2)
                        
                        sample_results[idx+ pre_sample] = sample(rows, idx+pre_sample)
                        
                        nid, eid, mfgs, root_nodes, ts, tsa, ret = sample_results[idx+1]
                        if idx==0:
                        ...
                        gather_future = process_executor.submit(gather, nid, eid, idx, raten, ratee)
                        
                        infer_future = executor.submit(infer_task, mfgsp, nf, ef, root_nodesp, tsp, idx, ret)
                        ...
                        print(f"Time taken for batch {idx} computation: {max(ti+tsa, tg+tc):.4f} seconds")
