...
def csr(list1, list2):
    ...
    # 计算相同元素的占比
    similarity_ratio = len(common_elements) / len(list2) if len(list2) > 0 else 0
...
def sample(rows, idx):
    ...
    if gnn_param['arch'] != 'identity':
        ...
        for id, b in enumerate(mfgs):
            ...
            print(f"nid no repeat:{len(set(nid[id]))}")
            print(f"eid no repeat:{len(set(eid[id]))}")
    ...
    # print(f"Time taken for gather {idx}: {tg:.4f} seconds")
...
def gather(nid, eid, idx):
    ...
    nf, ef = [], []
    for id in nid:
        nf.append(torch.zeros(len(id), gnn_dim_node))
        # nf.append(torch.zeros(len(id), gnn_dim_node).cuda())

    for id in eid:
        ef.append(torch.zeros(len(id), gnn_dim_edge))
...
def compute_aps(pred_pos, pred_neg, root_nodes, ts, idx, ret):
    ...
    print(f"Time taken for loss {idx}: {time_loss_end - time_loss_start:.4f} seconds")
    if mailbox is not None:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as process_executor:
                with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                    ...
                    for idx, rows in enumerate(get_batches(shuffled_df, train_param['batch_size'])):
                        ...
                        nidp1, eidp1, mfgsp1, root_nodesp1, tsp1, tsap1, retp1 = sample_results[idx]
                        nidp2, eidp2, mfgsp2, root_nodesp2, tsp2, tsap2, retp2 = sample_results[idx+1]
                        # print(f"node similarity: {csr(nidp1[0], nidp2[0])}")
                        # print(f"edge similarity: {csr(eidp1[0], eidp2[0])}")
                        ...
                        print(f"Time taken for batch {idx} computation: {max(ti+tsa, tg):.4f} seconds")
