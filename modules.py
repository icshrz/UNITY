...
class GeneralModel(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, sample_param, memory_param, gnn_param, train_param, combined=False):
        ...
        # self.w_q = torch.nn.Linear(dim_node + gnn_param['dim_time'], gnn_param['dim_out'],)
        # self.w_k = torch.nn.Linear(dim_node + dim_edge + gnn_param['dim_time'], gnn_param['dim_out'],)
        # self.w_v = torch.nn.Linear(dim_node + dim_edge + gnn_param['dim_time'], gnn_param['dim_out'],)
        ...
        if gnn_param['arch'] == 'transformer_attention':
            for h in range(sample_param['history']):
                self.layers['l0h' + str(h)] = TransformerAttentionLayer(self.dim_node_input, dim_edge, gnn_param['dim_time'], gnn_param['att_head'], train_param['dropout'], train_param['att_dropout'], gnn_param['dim_out'])
            for l in range(1, gnn_param['layer']):
                for h in range(sample_param['history']):
                    self.layers['l' + str(l) + 'h' + str(h)] = TransformerAttentionLayer(gnn_param['dim_out'], dim_edge, gnn_param['dim_time'], gnn_param['att_head'], train_param['dropout'], train_param['att_dropout'], gnn_param['dim_out'], combined=False)
    ...
    def forward(self, mfgs, neg_samples=1, Q_out=None, K_out=None, V_out=None, times = 1 ):
        ...
        for l in range(self.gnn_param['layer']):
            for h in range(self.sample_param['history']):
                rst = self.layers['l' + str(l) + 'h' + str(h)](mfgs[l][h], Q_out, K_out, V_out, times)
    ...
    def ret_wq(self):
        return self.layers['l0h0'].w_q
    
    def ret_wk(self):
        return self.layers['l0h0'].w_k
    
    def ret_wv(self):
        return self.layers['l0h0'].w_v
    
    def ret_time_zero(self, b):
        t = self.layers['l0h0'].time_enc(torch.zeros(b.num_dst_nodes(), dtype=torch.float32, device=torch.device('cuda:0')))
        return t
    
    def ret_time(self, b):
        t = self.layers['l0h0'].time_enc(b.edata['dt'])
        return t
