...
def prepare_input(mfgs, node_feats, edge_feats, combine_first=False, pinned=False, nfeat_buffs=None, efeat_buffs=None, nids=None, eids=None):
    if combine_first:
        for i in range(len(mfgs[0])):
            print(f"  Layer 0, Block {i}:")
            print(f"    Source Nodes: {torch.cat([mfgs[0][i].srcdata['ID'][:num_dst], unid], dim=0)} , Edges: {mfgs[0][i].edata['ID']}")
    ...
    if node_feats is not None:
        for b in mfgs[0]:
            if pinned:
                ...
                #print(f"    Node Features ('h') shape: {b.srcdata['h'].shape}")
                i += 1
            else:
            ...
            for b in mfg:
                if b.num_src_nodes() > b.num_dst_nodes():
                    if pinned:
                        ...
                        #print(f"    Edge Features ('h') shape: {b.edata['f'].shape}")
                        i += 1
                    else:
