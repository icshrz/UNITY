...
class ParallelTransformerAttentionLayer(torch.nn.Module):
    def __init__(self, dim_node_feat, dim_edge_feat, dim_time, num_head, dropout, att_dropout, dim_out, combined=False):
    ...
    def forward(self, b):
        ...
        # 分开计算每个不连续的张量
        results = []
        num_batches = 1000  # 每批次计算一个4KB的张量
        for i in range(num_batches):
            batch_b = b.clone()  # 创建一个批次副本
            batch_b.srcdata['h'] = batch_b.srcdata['h'][i:i+1]  # 提取该小批次
            batch_b.edata['f'] = batch_b.edata['f'][i:i+1]  # 同理处理边特征
            # 将每个小批次计算的结果保存
            result = self.compute_attention(batch_b, time_feat, zero_time_feat)
            results.append(result)
        
        return torch.cat(results, dim=0)  # 合并所有批次的结果

    def compute_attention(self, b, time_feat, zero_time_feat):
        Q = torch.zeros((b.num_edges(), self.dim_out), device=torch.device('cuda:0'))
        K = torch.zeros((b.num_edges(), self.dim_out), device=torch.device('cuda:0'))
        V = torch.zeros((b.num_edges(), self.dim_out), device=torch.device('cuda:0'))
